...
import { z } from "zod";
import { findLatestByExt, outputsDir, readLatestArtifacts } from "../util";
import type { ToolHandler } from "../../runtime/tools.js";

export const sendTelegram: ToolHandler = async (input, _deps) => {
  void _deps;
  ...
  if (!token || !chatId) {
    throw new Error(
      "Missing Telegram config. Set config/telegramon { botToken, chatId } to enable Telegram delivery.",
    );
  }
